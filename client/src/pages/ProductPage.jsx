...
const ProductPage = ({ navigate }) => {
  ...
  const [hasOrderedItem, setHasOrderedItem] = useState([]); // bool to check if the user has ordered this product
  ...
  }, [dispatch, successCreateReview, id]);
  ...
  useEffect(() => {
    if (product && product.reviews) {
      const sortdedArr = product.reviews.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
      );
      setAllReviews(sortdedArr);
    }
  }, [product]);
          ...
          <Row>
            <Col>
              ...
              {(!product.reviews || !product.reviews.length) && (
                <Message variant="secondary">No Revies Yet</Message>
              )}
              <ListGroup variant="flush">
                {allReviews.map((item) => {
                  return (
